...
class Dmail extends BaseScriptClass
{
    ...
    public function init()
    {
        ...
        if (!$this->params['use_domain']) {
            ...
            if ($rootLine) {
                ...
                if (BackendUtility::getDomainStartPage($parts['host'], $parts['path'])) {
                    ...
                    $domain = BackendUtility::getRecordsByField(
                        'sys_domain',
                        'domainName',
                        $temporaryPreUrl,
                        ' AND hidden=0',
        ...
        $this->MOD_MENU['dmail_mode'] = BackendUtility::unsetMenuItems(
            $this->params,
        ...
        if ($this->getLanguageService()->lang && ExtensionManagementUtility::isLoaded('static_info_tables')) {
            $res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
                ...
                'static_languages.lg_typo3=' . $GLOBALS['TYPO3_DB']->fullQuoteStr(
    ...
    public function mainAction(ServerRequestInterface $request, ResponseInterface $response)
    {
        ...
        $this->getLanguageService()->includeLLFile(
            'EXT:direct_mail/Resources/Private/Language/locallang_csh_sysdmail.xlf'
        );
    ...
    public function main()
    {
        ...
        if (($this->id && $access) || ($BE_USER->user['admin'] && !$this->id)) {
            ...
            $this->getPageRenderer()->addCssFile(
                ExtensionManagementUtility::extRelPath('direct_mail') . 'Resources/Public/StyleSheets/modules.css',
                'stylesheet',
                'all',
                '',
                false,
            ...
            $docHeaderButtons = [
                'PAGEPATH' => $this->getLanguageService()->sL(
                        'LLL:EXT:lang/locallang_core.php:labels.path'
                    ) . ': ' . GeneralUtility::fixed_lgd_cs($this->pageinfo['_thePath'], 50),
                'SHORTCUT' => '',
            ];
            // shortcut icon
            if ($BE_USER->mayMakeShortcut()) {
                $docHeaderButtons['SHORTCUT'] = $this->doc->makeShortcutIcon(
                    'id',
    ...
    public function compileQuickMail(array $row, $message)
    {
        ...
        if (!$message || !$htmlmail->theParts['plain']['content']) {
            ...
            $warningMsg .= '<br /><strong>' . $this->getLanguageService()->getLL(
                    'dmail_no_plain_boundaries'
                ) . '</strong>';
        }
        ...
        if (!$errorMsg) {
            ...
            if ($warningMsg) {
                return $this->doc->section(
                    $this->getLanguageService()->getLL('dmail_warning'),
                    $warningMsg . '<br /><br />'
                );
            }
        }
        ...
        switch ($this->CMD) {
            case 'info':
                ...
                if ($createMailFromInternalPage && !$quickmail['send']) {
                    ...
                    $newUid = DirectMailUtility::createDirectMailRecordFromPage(
                        $createMailFromInternalPage,
                        $this->params,
                        $createMailFromInternalPageLang
                    );
                    ...
                    if ($row['type'] == '1' && ((empty($row['HTMLParams'])) || (empty($row['plainParams'])))) {
                        ...
                        $theOutput .= $this->compileQuickMail(
                            $row,
                            $unserializedMailContent['plain']['content'],
                            false
                        );
                    } else {
                        if ($shouldFetchData) {
                ...
                $navigationButtons = '<input type="submit" class="btn btn-default" value="' . $this->getLanguageService(
                    )->getLL('dmail_wiz_back') . '" name="back"> &nbsp;';
                $navigationButtons .= '<input type="submit" value="' . $this->getLanguageService()->getLL(
                        'dmail_wiz_next'
                    ) . '" ' . ($fetchError ? 'disabled="disabled" class="next btn btn-default disabled"' : ' class="btn btn-default"') . '>';
            ...
            default:
                ...
                $this->currentStep = 1;
                $markers['TITLE'] = $this->getLanguageService()->getLL(
                        'dmail_wiz1_new_newsletter'
                    ) . ' - ' . $this->getLanguageService()->getLL('dmail_wiz1_select_nl_source');
    ...
    public function cmd_finalmail($direct_mail_row)
    {
        ...
        while (($group = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res))) {
            ...
            $opt[] = '<option value="' . $group['uid'] . '">' . htmlspecialchars(
                    $group['title'] . ' (#' . $count . ')'
                ) . '</option>';
            $lastGroup = $group;
        }
        ...
        if (count($opt) === 0) {
            ...
            $groupInput = '<select multiple="multiple" name="mailgroup_uid[]" ' . ($hookSelectDisabled ? 'disabled' : '') . '>' . implode(
                    chr(10),
                    $opt
                ) . '</select>';
        }
        ...
        $msg .= $this->getLanguageService()->getLL('schedule_time') .
            ...
            '<input class="form-control t3js-datetimepicker t3js-clearable" data-date-type="datetime" data-date-offset="0" type="text" id="tceforms-datetimefield-startdate" name="send_mail_datetime_hr" value="' . strftime(
                '%H:%M %d-%m-%Y',
                time()
            ) . '">' .
        ...
        $msg .= '<br/><label for="tx-directmail-sendtestmail-check"><input type="checkbox" name="testmail" id="tx-directmail-sendtestmail-check" value="1" />&nbsp;' . $this->getLanguageService(
            )->getLL('schedule_testmail') . '</label>';
        $msg .= '<br/><label for="tx-directmail-savedraft-check"><input type="checkbox" name="savedraft" id="tx-directmail-savedraft-check" value="1" />&nbsp;' . $this->getLanguageService(
            )->getLL('schedule_draft') . '</label>';
        $msg .= '<br /><br /><input class="btn btn-default" type="Submit" name="mailingMode_mailGroup" value="' . $this->getLanguageService(
            )->getLL('schedule_send_all') . '" />';

        $theOutput = $this->doc->section(
            $this->getLanguageService()->getLL('schedule_select_mailgroup'),
            $msg,
            1,
            1,
            0,
        ...
        if ($simpleMailMode) {
            ...
            if (GeneralUtility::_GP('tt_address_uid')) {
                ...
                if (($recipRow = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res))) {
                    ...
                    $recipRow['sys_dmail_categories_list'] = $htmlmail->getListOfRecipentCategories(
                        'tt_address',
                        $recipRow['uid']
                    );
                    ...
                    $flashMessage = GeneralUtility::makeInstance(
                        'TYPO3\\CMS\\Core\\Messaging\\FlashMessage',
                        sprintf(
                            $this->getLanguageService()->getLL('send_was_sent_to_name'),
                            htmlspecialchars($recipRow['name']) . htmlspecialchars(' <' . $recipRow['email'] . '>')
                        ),
                        $this->getLanguageService()->getLL('send_sending'),
                        FlashMessage::OK
                    );
            ...
            if (GeneralUtility::_GP('mailingMode_mailGroup') && $this->sys_dmail_uid && is_array($recipientGroups)) {
                ...
                if (GeneralUtility::_GP('savedraft')) {
                    ...
                    $content = $this->getLanguageService()->getLL(
                            'send_was_scheduled_for'
                        ) . ' ' . BackendUtility::datetime($distributionTime);
    ...
    public function cmd_testmail()
    {
        ...
        if ($this->params['test_tt_address_uids']) {
            ...
            $GLOBALS['TYPO3_DB']->sql_free_result($res);
            $msg .= $this->getRecordList(
                DirectMailUtility::fetchRecordsListValues($ids, 'tt_address'),
                'tt_address',
                1,
                1
            );
        ...
        if ($this->params['test_dmail_group_uids']) {
            ...
            while (($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res))) {
                $msg .= '<a href="' . BackendUtility::getModuleUrl(
                        'DirectMailNavFrame_DirectMail'
                    ) . '&id=' . $this->id . '&sys_dmail_uid=' . $this->sys_dmail_uid . '&CMD=send_mail_test&sys_dmail_group_uid[]=' . $row['uid'] . '">' .
        ...
        $msg .= '<input' . $GLOBALS['TBE_TEMPLATE']->formWidth(
            ) . ' type="text" name="SET[dmail_test_email]" value="' . $this->MOD_SETTINGS['dmail_test_email'] . '" /><br /><br />';

        $msg .= '<input type="hidden" name="id" value="' . $this->id . '" />';
        $msg .= '<input type="hidden" name="sys_dmail_uid" value="' . $this->sys_dmail_uid . '" />';
        $msg .= '<input type="hidden" name="CMD" value="send_mail_test" />';
        $msg .= '<input type="submit" name="mailingMode_simple" value="' . $this->getLanguageService()->getLL(
                'dmail_send'
            ) . '" />';
    ...
    public function cmd_displayMailGroup_test($result)
    {
        ...
        $out = '';
        if (is_array($idLists['tt_address'])) {
            $out .= $this->getRecordList(
                DirectMailUtility::fetchRecordsListValues($idLists['tt_address'], 'tt_address'),
                'tt_address'
            );
        }
        if (is_array($idLists['fe_users'])) {
            $out .= $this->getRecordList(
                DirectMailUtility::fetchRecordsListValues($idLists['fe_users'], 'fe_users'),
                'fe_users'
            );
        }
        if (is_array($idLists['PLAINLIST'])) {
        ...
        if (is_array($idLists[$this->userTable])) {
            $out .= $this->getRecordList(
                DirectMailUtility::fetchRecordsListValues($idLists[$this->userTable], $this->userTable),
                $this->userTable
            );
        }
    ...
    public function getRecordList(array $listArr, $table, $editLinkFlag = 1, $testMailLink = 0)
    {
        ...
        if (is_array($listArr)) {
            ...
            foreach ($listArr as $row) {
                ...
                if ($row['uid']) {
                    $tableIcon = '<td>' . $this->iconFactory->getIconForRecord(
                            $table,
                            $row,
                            Icon::SIZE_SMALL
                        ) . '</td>';
                    if ($editLinkFlag) {
                        $requestUri = GeneralUtility::getIndpEnv(
                                'REQUEST_URI'
                            ) . '&CMD=send_test&sys_dmail_uid=' . $this->sys_dmail_uid . '&pages_uid=' . $this->pages_uid;
                        $editLink = '<td><a href="#" onClick="' . BackendUtility::editOnClick(
                                '&edit[tt_address][' . $row['uid'] . ']=edit',
                                $GLOBALS['BACK_PATH'],
                                $requestUri
                            ) . '" title="' . $this->getLanguageService()->getLL("dmail_edit") . '">' .
                            $this->iconFactory->getIcon('actions-open', Icon::SIZE_SMALL) .
                            '</a></td>';
                    }

                    if ($testMailLink) {
                        $testLink = '<a href="' . BackendUtility::getModuleUrl(
                                'DirectMailNavFrame_DirectMail'
                            ) . '&id=' . $this->id . '&sys_dmail_uid=' . $this->sys_dmail_uid . '&CMD=send_mail_test&tt_address_uid=' . $row['uid'] . '">' . htmlspecialchars(
                                $row['email']
                            ) . '</a>';
                    } else {
        ...
        if (count($lines)) {
            ...
            $out .= '<table border="0" cellspacing="1" cellpadding="0" class="table table-striped table-hover">' . implode(
                    LF,
                    $lines
                ) . '</table>';
        }
                ...
                switch ($mailGroup['type']) {
                    case 0:
                            ...
                            if ($pageUid > 0) {
                                ...
                                if (is_array($pageinfo)) {
                                    ...
                                    if ($mailGroup['recursive']) {
                                        $pageIdArray = array_merge(
                                            $pageIdArray,
                                            DirectMailUtility::getRecursiveSelect(
                                                $pageUid,
                                                $this->perms_clause
                                            )
                                        );
                        ...
                        if ($pidList) {
                            ...
                            if ($whichTables & 1) {
                                // tt_address
                                $idLists['tt_address'] = DirectMailUtility::getIdList(
                                    'tt_address',
                                    $pidList,
                                    $groupUid,
                            ...
                            if ($whichTables & 2) {
                                // fe_users
                                $idLists['fe_users'] = DirectMailUtility::getIdList(
                                    'fe_users',
                                    $pidList,
                                    $groupUid,
                                    $mailGroup['select_categories']
                            ...
                            if ($this->userTable && ($whichTables & 4)) {
                                // user table
                                $idLists[$this->userTable] = DirectMailUtility::getIdList(
                                    $this->userTable,
                                    $pidList,
                                    $groupUid,
                                    $mailGroup['select_categories']
                            ...
                            if ($whichTables & 8) {
                                ...
                                $idLists['fe_users'] = array_unique(
                                    array_merge(
                                        $idLists['fe_users'],
                                        DirectMailUtility::getIdList(
                                            'fe_groups',
                                            $pidList,
                                            $groupUid,
                                            $mailGroup['select_categories']
                    ...
                    case 1:
                        // List of mails
                        if ($mailGroup['csv'] == 1) {
                            $recipients = DirectMailUtility::rearrangeCsvValues(
                                DirectMailUtility::getCsvValues($mailGroup['list']),
                                $this->fieldList
                            );
                        } else {
                            $recipients = DirectMailUtility::rearrangePlainMails(
                                array_unique(preg_split('|[[:space:],;]+|', $mailGroup['list']))
                            );
                        }
                    ...
                    case 2:
                        ...
                        $idLists['fe_users'] = array_unique(
                            array_merge(
                                $idLists['fe_users'],
                                DirectMailUtility::getStaticIdList('fe_groups', $groupUid)
                            )
                        );
                        if ($this->userTable) {
                            $idLists[$this->userTable] = DirectMailUtility::getStaticIdList(
                                $this->userTable,
                                $groupUid
                            );
                        }
                    ...
                    case 3:
                        ...
                        if ($table) {
                            ...
                            $idLists[$table] = DirectMailUtility::getSpecialQueryIdList(
                                $queryGenerator,
                                $table,
                                $mailGroup
                            );
                    ...
                    case 4:
                        $groups = array_unique(
                            DirectMailUtility::getMailGroups(
                                $mailGroup['mail_groups'],
                                [$mailGroup['uid']],
                                $this->perms_clause
                            )
                        );
    ...
    public function makeCategoriesForm(array $row)
    {
        ...
        if (!$GLOBALS['TYPO3_DB']->sql_num_rows($res)) {
            $theOutput = $this->doc->section(
                $this->getLanguageService()->getLL('nl_cat'),
                $this->getLanguageService()->getLL('nl_cat_msg1'),
                1,
                1,
                0,
                true
            ...
            while (($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res))) {
                ...
                if ($colPosVal != $row['colPos']) {
                    $out .= '<tr><td colspan="3" bgcolor="' . $this->doc->bgColor5 . '">' . $this->getLanguageService(
                        )->getLL('nl_l_column') . ': <strong>' . BackendUtility::getProcessedValue(
                            'tt_content',
                            'colPos',
                            $row['colPos']
                        ) . '</strong></td></tr>';
                ...
                $out .= '<tr>';
                $out .= '<td valign="top" width="75%">' . $this->iconFactory->getIconForRecord(
                        'tt_content',
                        $row,
                        Icon::SIZE_SMALL
                    ) .
                    $row['header'] . '<br />' . GeneralUtility::fixed_lgd_cs(
                ...
                if ($row['module_sys_dmail_category']) {
                    $checkBox .= '<strong style="color:red;">' . $this->getLanguageService()->getLL(
                            'nl_l_ONLY'
                        ) . '</strong>';
                } else {
                    $checkBox .= '<strong style="color:green">' . $this->getLanguageService()->getLL(
                            'nl_l_ALL'
                        ) . '</strong>';
                }
                $checkBox .= '<br />';
                ...
                foreach ($this->categories as $pKey => $pVal) {
                    $checkBox .= '<input type="hidden" name="indata[categories][' . $row["uid"] . '][' . $pKey . ']" value="0">' .
                        '<input type="checkbox" name="indata[categories][' . $row['uid'] . '][' . $pKey . ']" value="1"' . (GeneralUtility::inList(
                            $categoriesRow,
                            $pKey
                        ) ? ' checked' : '') . '> ' .
            ...
            $out .= '<input type="hidden" name="pages_uid" value="' . $this->pages_uid . '">' .
                ...
                '<input type="submit" name="update_cats" value="' . $this->getLanguageService()->getLL(
                    'nl_l_update'
                ) . '">';

            $theOutput = $this->doc->section(
                $this->getLanguageService()->getLL('nl_cat') . BackendUtility::cshItem(
                    $this->cshTable,
                    'assign_categories',
                    $GLOBALS['BACK_PATH']
                ),
    ...
    public function makeFormInternal($boxId, $totalBox, $open = false)
    {
        ...
        $output .= '<a href="#" onclick="toggleDisplay(\'' . $boxId . '\', event, ' . $totalBox . ')">' . $imgSrc . $this->getLanguageService(
            )->getLL('dmail_wiz1_internal_page') . '</a>';
    ...
    public function makeFormExternal($boxId, $totalBox, $open = false)
    {
        ...
        $output = '<div class="box"><div class="toggleTitle">';
        $output .= '<a href="#" onclick="toggleDisplay(\'' . $boxId . '\', event, ' . $totalBox . ')">' . $imgSrc . $this->getLanguageService(
            )->getLL('dmail_wiz1_external_page') . '</a>';
        $output .= '</div><div id="' . $boxId . '" class="toggleBox" style="display:' . ($open ? 'block' : 'none') . '">';

        // Create
        $out = $this->getLanguageService()->getLL('dmail_HTML_url') . '<br />
				<input type="text" value="http://" name="createMailFrom_HTMLUrl"' . $GLOBALS['TBE_TEMPLATE']->formWidth(
                40
            ) . ' /><br />' .
            $this->getLanguageService()->getLL('dmail_plaintext_url') . '<br />
				<input type="text" value="http://" name="createMailFrom_plainUrl"' . $GLOBALS['TBE_TEMPLATE']->formWidth(
                40
            ) . ' /><br />' .
            $this->getLanguageService()->getLL('dmail_subject') . '<br />' .
            '<input type="text" value="' . $this->getLanguageService()->getLL(
                'dmail_write_subject'
            ) . '" name="createMailFrom_URL" onFocus="this.value=\'\';"' . $GLOBALS['TBE_TEMPLATE']->formWidth(
                40
            ) . ' /><br />' .
            (($this->error == 'no_valid_url') ? ('<br /><b>' . $this->getLanguageService()->getLL(
                    'dmail_no_valid_url'
                ) . '</b><br /><br />') : '') .
            '<input type="submit" value="' . $this->getLanguageService()->getLL("dmail_createMail") . '" />
				<input type="hidden" name="fetchAtOnce" value="1">';
        $output .= '<h3>' . $this->getLanguageService()->getLL('dmail_dovsk_crFromUrl') . BackendUtility::cshItem(
                $this->cshTable,
                'create_directmail_from_url',
                $GLOBALS['BACK_PATH']
            ) . '</h3>';
        $output .= $out;
    ...
    public function makeListDMail($boxId, $totalBox, $open = false)
    {
        ...
        while (($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res))) {
            $tblLines[] = [
                ...
                ($row['issent'] ? $this->getLanguageService()->getLL('dmail_yes') : $this->getLanguageService()->getLL(
                    'dmail_no'
                )),
                ...
                ($row['type'] & 0x1 ? $this->getLanguageService()->getLL('nl_l_tUrl') : $this->getLanguageService(
                )->getLL('nl_l_tPage')) . ($row['type'] & 0x2 ? ' (' . $this->getLanguageService()->getLL(
                        'nl_l_tDraft'
                    ) . ')' : ''),
    ...
    public function cmd_quickmail()
    {
        ...
        $theOutput .= $this->getLanguageService()->getLL(
                'quickmail_sender_name'
            ) . '<br /><input type="text" name="quickmail[senderName]" value="' . htmlspecialchars(
                $senderName
            ) . '"' . $this->doc->formWidth() . ' /><br />';
        $theOutput .= $this->getLanguageService()->getLL(
                'quickmail_sender_email'
            ) . '<br /><input type="text" name="quickmail[senderEmail]" value="' . htmlspecialchars(
                $senderMail
            ) . '"' . $this->doc->formWidth() . ' /><br />';
        $theOutput .= $this->getLanguageService()->getLL(
                'dmail_subject'
            ) . '<br /><input type="text" name="quickmail[subject]" value="' . htmlspecialchars(
                $indata['subject']
            ) . '"' . $this->doc->formWidth() . ' /><br />';
        $theOutput .= $this->getLanguageService()->getLL(
                'quickmail_message'
            ) . '<br /><textarea rows="20" name="quickmail[message]"' . $this->doc->formWidth(
            ) . '>' . LF . htmlspecialchars($indata['message']) . '</textarea><br />';
        $theOutput .= $this->getLanguageService()->getLL(
                'quickmail_break_lines'
            ) . ' <input type="checkbox" name="quickmail[breakLines]" value="1"' . ($indata['breakLines'] ? ' checked="checked"' : '') . ' /><br /><br />';
        $theOutput .= '<input type="Submit" name="quickmail[send]" value="' . $this->getLanguageService()->getLL(
                'dmail_wiz_next'
            ) . '" />';
    ...
    public function cmd_news()
    {
        ...
        if (!$GLOBALS['TYPO3_DB']->sql_num_rows($res)) {
            $theOutput = $this->doc->section(
                ...
                $this->getLanguageService()->getLL('nl_select_msg1'),
                0,
                1
            );
        } else {
            ...
            while (($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res))) {
                ...
                $createDmailLink = BackendUtility::getModuleUrl(
                        'DirectMailNavFrame_DirectMail'
                    ) . '&id=' . $this->id . '&createMailFrom_UID=' . $row['uid'] . '&fetchAtOnce=1&CMD=info';
                ...
                foreach ($languages as $languageUid => $lang) {
                    ...
                    $htmlIcon = $this->iconFactory->getIcon(
                        'direct_mail_preview_html',
                        Icon::SIZE_SMALL,
                        $langIconOverlay
                    );
                    $plainIcon = $this->iconFactory->getIcon(
                        'direct_mail_preview_plain',
                        Icon::SIZE_SMALL,
                        $langIconOverlay
                    );
                    $createIcon = $this->iconFactory->getIcon(
                        'direct_mail_newmail',
                        Icon::SIZE_SMALL,
                        $langIconOverlay
                    );
                    ...
                    $previewHTMLLink .= '<a href="#" onClick="' . BackendUtility::viewOnClick(
                            ...
                            $GLOBALS['LANG']->getLL('nl_viewPage_HTML') . $langTitle
                        ) . '">' . $htmlIcon . '</a>';
                    $previewTextLink .= '<a href="#" onClick="' . BackendUtility::viewOnClick(
                            ...
                            $GLOBALS['LANG']->getLL('nl_viewPage_TXT') . $langTitle
                        ) . '">' . $plainIcon . '</a>';
                    $createLink .= '<a href="' . $createDmailLink . $createLangParam . '" title="' . htmlentities(
                            $GLOBALS['LANG']->getLL("nl_create") . $langTitle
                        ) . '">' . $createIcon . '</a>';
                }
                ...
                $outLines[] = [
                    ...
                    $createLink,
                    '<a onclick="' . htmlspecialchars(
                        BackendUtility::editOnClick('&edit[pages][' . $row['uid'] . ']=edit', $this->doc->backPath)
                    ) . '" href="#" title="' . $GLOBALS['LANG']->getLL(
                        "nl_editPage"
            ...
            $theOutput = $this->doc->section(
                $this->getLanguageService()->getLL('dmail_dovsk_crFromNL') . BackendUtility::cshItem(
                    $this->cshTable,
                    'select_newsletter',
                    $GLOBALS['BACK_PATH']
                ),
    ...
    {
        return '<a class="t3-link" href="' . BackendUtility::getModuleUrl(
                'DirectMailNavFrame_DirectMail'
            ) . '&id=' . $this->id . '&sys_dmail_uid=' . $uid . '&CMD=info&fetchAtOnce=1">' . htmlspecialchars(
                $str
            ) . '</a>';
    }
    ...
    protected function renderRecordDetailsTable(array $row)
    {
        if (!$row['issent']) {
            if ($GLOBALS['BE_USER']->check('tables_modify', 'sys_dmail')) {
                ...
                $requestUri = BackendUtility::getModuleUrl(
                        'DirectMailNavFrame_DirectMail'
                    ) . '&id=' . $this->id . '&CMD=info&sys_dmail_uid=' . $row['uid'] . '&fetchAtOnce=1';

                $editParams = BackendUtility::editOnClick(
                    '&edit[sys_dmail][' . $row['uid'] . ']=edit',
                    $GLOBALS['BACK_PATH'],
                    $requestUri
                );

                $content = '<a href="#" onClick="' . $editParams . '" title="' . $this->getLanguageService()->getLL(
                        "dmail_edit"
                    ) . '">' .
                    $this->iconFactory->getIcon('actions-open', Icon::SIZE_SMALL) .
                    '<b>' . $this->getLanguageService()->getLL('dmail_edit') . '</b></a>';
            } else {
                $content = $this->iconFactory->getIcon(
                        'actions-open',
                        Icon::SIZE_SMALL
                    ) . ' (' . $this->getLanguageService()->getLL('dmail_noEdit_noPerms') . ')';
            }
        } else {
            $content = $this->iconFactory->getIcon('actions-open', Icon::SIZE_SMALL) . '(' . $this->getLanguageService(
                )->getLL('dmail_noEdit_isSent') . ')';
        }

        $content = '<thead >
			<th>' . DirectMailUtility::fName('subject') . ' <b>' . GeneralUtility::fixed_lgd_cs(
        ...
        $nameArr = explode(
            ',',
            'from_name,from_email,replyto_name,replyto_email,organisation,return_path,priority,attachment,type,page,sendOptions,includeMedia,flowedFormat,sys_language_uid,plainParams,HTMLParams,encoding,charset,issent,renderedsize'
        );
        ...
        $sectionTitle = $this->iconFactory->getIconForRecord('sys_dmail', $row, Icon::SIZE_SMALL)->render(
            ) . '&nbsp;' . htmlspecialchars($row['subject']);
    ...
    public function deleteLink($uid)
    {
        ...
        if (!$dmail['scheduled_begin']) {
            return '<a href="' . BackendUtility::getModuleUrl(
                    'DirectMailNavFrame_DirectMail'
                ) . '&id=' . $this->id . '&CMD=delete&uid=' . $uid . '">' . $icon . '</a>';
        }
